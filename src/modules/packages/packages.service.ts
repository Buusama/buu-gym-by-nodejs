...
export class PackagesService extends PageService {
  ...
  async updatePackage(
    ...
    const existingPackage = await this.packagesRepository.findOneByOrFail({
      id: id,
    });
    const { ...params } = updatePackageDto;

    this.packagesRepository.merge(existingPackage, params);
    await this.packagesRepository.save(existingPackage);
    return this.getPackage(id);
  }

  async deletePackage(id: number): Promise<PageResponseDto<Package>> {
    const existingPackage = await this.packagesRepository.findOneByOrFail({
      id: id,
    });
    
    const deletedPackage = await this.packagesRepository.remove(existingPackage);
    this.packagesRepository.save(deletedPackage);
    return new PageResponseDto(existingPackage);
  }
}
